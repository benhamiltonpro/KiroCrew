...
`allowedTools` where the governance ceiling permits. Carrying the pointer is the
whole of what this path does to a ref list — it adds, and it removes nothing. A
reference for a server no scope declares at all matches neither the sync's
disabled branch nor its add branch, so this path leaves it alone; [the final ref
reconcile](#the-final-ref-reconcile) is what then drops it, over the final map and
for its own reason. A pointer is not exempt from that reconcile and does not need
to be: carrying it puts its name in the map, which is exactly the test the
reconcile applies.
